<dialog id="add_event_modal" style="padding: 2rem; border-radius: 0.2rem;">
    <?php $wedding_event = query("SELECT * from calendar_items WHERE name = 'Wedding Ceremony';")[0]; ?>
    <form method="POST" action="/handle_add_event">
        <h3 class="title"><?= $wedding_event ? 'Add Event' : 'Add Your Ceremony' ?></h3>
        <hr>
        ...
        <?php } else {?>
            <input type="hidden" name="name" value="Wedding Ceremony">
        <?php } ?>
